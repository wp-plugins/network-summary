...
class Network_Summary_Admin
{
    ...
    public function update_network_settings() {
        ...
        $options = get_site_option( NETWORK_SUMMARY_OPTIONS );
        var_dump( $options );
        if ( isset( $_POST['deciding_role'] ) && in_array( $_POST['deciding_role'], array( 'site_admin', 'network_admin' ) ) ) {
            $options['deciding_role'] = $_POST['deciding_role'];
        }

        if ( isset( $_POST['rss_limit'] ) && is_numeric( $_POST['rss_limit'] )
            && $_POST['rss_limit'] > 0 && $_POST['rss_limit'] <= 1000
        ) {
            $options['rss_limit'] = (int)$_POST['rss_limit'];
        }
        update_site_option( NETWORK_SUMMARY_OPTIONS, $options );
