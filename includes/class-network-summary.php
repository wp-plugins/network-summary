...
class Network_Summary
{
    ...
    public function activate() {
        foreach ( $this->get_sites() as $site_id ) {
            add_blog_option( $site_id, NETWORK_SUMMARY_OPTIONS, $this->get_default_site_values()
            );
        }
        add_site_option( NETWORK_SUMMARY_OPTIONS, $this->get_default_network_values() );
        add_site_option( 'network_summary_version', NETWORK_SUMMARY_VERSION );

        $this->site_categories->create_table();
    }

    private function get_default_site_values() {
        return array(
            'share_site' => '0',
            'site_description' => ''
        );
    ...
    private function get_default_network_values( $tag = null ) {
        return array(
    ...
    public function maybe_update() {
        ...
        switch ( $old_version ) {
            ...
            case '2.0.1':
            case '2.0.2':
                $site_options = get_site_option( NETWORK_SUMMARY_OPTIONS );
                $defaults = $this->get_default_network_values();
                $site_options['rss_limit'] = $defaults['rss_limit'];
                update_site_option( NETWORK_SUMMARY_OPTIONS, $site_options );
        }
