...
if ( isset( $sites_table ) ) :
    ...
    $deciding_role = $network_summary->get_option( 'deciding_role', true );
    $rss_limit = $network_summary->get_option( 'rss_limit', true );
        ...
        <table class="form-table">
            ...
            <tr>
                <th class="label-row-deciding-role"><label
                        for="deciding_role"><?php _e( 'Deciding Role on Sharing Content', 'network-summary' ); ?></label>
                </th>
                <td>

                    <select name="deciding_role" required>
                        <option <?php selected( $deciding_role, 'site_admin', true ) ?>
                            value="site_admin">
                            <?php _e( 'Site Admin', 'network-summary' ); ?>
                        </option>
                        <option <?php selected( $deciding_role, 'network_admin', true ) ?>
            ...
            <tr>
                <th class="label-rss-limit"><label
                        for="rss_limit"><?php _e( 'RSS Feed Limit', 'network-summary' ); ?></label></th>
                <td>
                    <input name="rss_limit" type="number" min="1" max="1000" required="required"
